    ...
    <div className="max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-10">
        ...
        >
          <Card className="bg-zinc-900/40 border-white/5 backdrop-blur-md shadow-xl hover:bg-zinc-900/60 transition-colors">
            <CardHeader className="flex flex-row items-center justify-between pb-2">
              <CardTitle className="text-sm font-medium text-zinc-400">Faturamento Bruto (Volume de Vendas)</CardTitle>
            ...
            <CardContent>
              ...
              {vendas.length > 0 && (
                <p className="text-xs text-emerald-400 mt-2 font-medium bg-emerald-500/10 inline-block px-2 py-1 rounded">
        ...
        >
          <Card className="bg-zinc-900/40 border-white/5 backdrop-blur-md shadow-xl hover:bg-zinc-900/60 transition-colors">
            <CardHeader className="flex flex-row items-center justify-between pb-2">
              <CardTitle className="text-sm font-medium text-zinc-400">Lucro Real (Faturamento - Aquisição)</CardTitle>
        ...
        >
          <Card className="bg-zinc-900/40 border-white/5 backdrop-blur-md shadow-xl hover:bg-zinc-900/60 transition-colors">
            <CardHeader className="flex flex-row items-center justify-between pb-2">
              <CardTitle className="text-sm font-medium text-zinc-400">Giro de Estoque</CardTitle>
            ...
            <CardContent>
              <div className="flex items-baseline gap-2">
                 <div className="text-3xl font-bold text-white">{totalAmpolasAtivas}</div>
                 <span className="text-sm font-medium text-zinc-500">ampolas ativas</span>
              </div>
              {ampolasCriticAS > 0 && (
                <p className="text-xs text-indigo-400 mt-2 font-medium bg-indigo-500/10 inline-block px-2 py-1 rounded">
      ...
      >
        <Card className="bg-black/40 border-white/5 p-6 backdrop-blur-2xl shadow-2xl">
          ...
          {chartData.length === 0 ? (
             <div className="h-[450px] w-full flex items-center justify-center text-zinc-500 flex-col gap-3">
               <Activity size={48} className="opacity-20 text-white" />
               <p>Aguardando as primeiras vendas entrarem no sistema para gerar o Analytics.</p>
             </div>
          ) : (
            <div className="h-[450px] w-full min-h-[450px]">
              <ResponsiveContainer width="100%" height="100%" minWidth={100} minHeight={450}>
                <LineChart data={chartData} margin={{ top: 5, right: 20, left: -20, bottom: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#27272a" vertical={false} />
                  <XAxis dataKey="name" stroke="#52525b" tick={{fill: '#a1a1aa'}} axisLine={false} tickLine={false} dy={10} />
                  <YAxis stroke="#52525b" tick={{fill: '#a1a1aa'}} axisLine={false} tickLine={false} tickFormatter={(val) => `R$${val/1000}k`} />
                  <Tooltip 
                    ...
                    contentStyle={{ backgroundColor: '#09090b', borderColor: '#27272a', borderRadius: '12px', boxShadow: '0 10px 15px -3px rgb(0 0 0 / 0.5)' }}
                    itemStyle={{color: '#e4e4e7'}}
                    labelStyle={{color: '#a1a1aa', fontWeight: 'bold', marginBottom: '8px'}}
                  />
                  <Line type="step" dataKey="lucro_bruto" name="L. Bruto (Venda)" stroke="#34d399" strokeWidth={3} dot={{r: 4, strokeWidth: 2, fill: '#09090b'}} activeDot={{r: 6, stroke: '#34d399', strokeWidth: 2}} />
                  <Line type="monotone" dataKey="lucro_liquido" name="L. Líquido Real" stroke="#f59e0b" strokeWidth={3} dot={{r: 4, strokeWidth: 2, fill: '#09090b'}} activeDot={{r: 6, stroke: '#f59e0b', strokeWidth: 2}} />
                  <Line type="monotone" dataKey="custo" name="Custo Base" stroke="#ef4444" strokeWidth={2} strokeDasharray="5 5" dot={false} activeDot={{r: 4}} />
                </LineChart>
              </ResponsiveContainer>
            </div>
          )}
